...
public class HelloWorld implements FunctionInterface {
    ...
    public void function(
        ...
        String payload = new String(np.getPayload(),UTF8_CHARSET); 
        
        NodeAddress tmp = np.getDst();
        np.setDst(np.getSrc());
        np.setSrc(tmp);
        np.setPayload((payload.toUpperCase(Locale.ENGLISH))
                .getBytes(UTF8_CHARSET));
        flowTableQueue.add(np.toIntArray());
    }   
}